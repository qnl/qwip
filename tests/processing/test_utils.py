...
class TestFormatLegacyHeterodyne:    
    ...
    def meas(self):
        return {
            ...
            'R1': {
                'Heterodyne': np.arange(2*256*20*1).reshape((2, 256, 20, 1))
            }
        }
...
class TestFilterData:
    ...
    def psettings(self):
        psettings = ProcessSettings(
            name='filter', process_type='utils.FilterData'
        )

        return psettings

    @pytest.mark.parametrize('regex,keys', [
        ('R1', ['R1']),
    ...
    @pytest.mark.parametrize('container,keys', [
        (['R1'], ['R1']),
        (['Q1', 'R1'], ['R1']),
        (['Q1'], [])
    ])
...
class TestCollectData:
    ...
    def psettings(self):
        psettings = ProcessSettings(
            name='collect', process_type='utils.CollectData'
        )
