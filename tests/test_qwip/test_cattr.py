...
class TestBuiltins:
    ...
    def test_set(self, tp, obj, expected):
        unstructured = qwip.converter.unstructure(obj, unstructure_as=tp)
...
class TestFlatDict:
    ...
    @pytest.mark.parametrize(
        "obj", [{}, {"a": 0, "b": 1}, {"a": "0", "b": 2}, {"a": True, "b": False}]
    )
    ...
    @pytest.mark.parametrize("cls", [FlatDict, Optional[FlatDict], FlatDict[str, Any]])
    @pytest.mark.parametrize("obj", NESTED_OBJS)
    ...
    @pytest.mark.parametrize(
        "obj", [({"k1": {"a": 1}, "k2": {"a": 1}}), ({"k1": {"a": 1, "b": {"k": 2}}})]
    )
...
class TestNumpy:
    ...
    @pytest.mark.parametrize("cls", [NDArray, Optional[NDArray]])
