...
@qdefine
class WaveformBlock:
    """Intermediate representation of a scheduled and compiled set of waveforms.

    Each waveform block represents a single SequenceElement or timeline of waveforms,
    with no additional control flow, containing the data for each channel on a single
    device.

    Attributes:
        samples: The total number of samples in the waveform block.
        sample_rate: The sampling rate for all waveforms in the block.
        waveforms: A mapping of channel index, subchannel index pairs to a numpy array
            containing the waveform data for that channel.
        markers: A list of sample indices for each marker that is played on a channel.
    """

    samples: int
    sample_rate: float
    waveforms: dict[tuple[int, int], np.ndarray] = field(factory=dict)
    markers: list[int] = field(factory=list)
    triggered: bool = False
    ...
    def empty(
        ...
        dtype: type | list[type] = np.float32,
    ) -> Self:
        if isinstance(dtype, type):
            dtype = [dtype] * len(channels)
        elif len(dtype) != len(channels):
            raise ValueError("The number of data types and channels must be equal!")

        waveforms = {
            (ch.index, ch.subchannel): np.zeros(samples, dtype=tp)
            for ch, tp in zip(channels, dtype)
        }
    ...
    def add_waveform(
        self, channel: ChannelInfo, dtype: type = np.float32
    ) -> np.ndarray:
        arr = self.waveforms[channel.index, channel.subchannel] = np.zeros(
            self.samples, dtype=dtype
        )
    ...
    def get_waveform(
        ...
        try:
            ...
            return self.add_waveform(channel, dtype)

    def __getitem__(self, key) -> np.ndarray:
        match key:
            case int():
                return self.waveforms[key, 0]
            case ChannelInfo(index=idx, subchannel=sc):
                return self.waveforms[idx, sc]
            case _:
...
class QWiPExecutable(QuantumExecutable):
    """An intermediate representation for scheduled and compiled sequences.

    Attributes:
        devices: A mapping from device to a
    """

    devices: dict[str, list[WaveformBlock]] = field(factory=dict)
    triggers: dict[str, list[TriggerBlock]] = field(factory=dict)

    def plot(self, element: int) -> Figure:
        fig, ax = plt.subplots()

        for dev, seq in self.devices.items():
            try:
                wblk = seq[element]
            except IndexError:
                continue
...
class QWiPSequencer:
    ...
    The base waveform sequencer is responsible for scheduling pulses within a sequence
    element and compiling waveforms into concrete timepoints that can then be converted
    to a hardware specific executable format and uploaded to the measurement hardware.
    ...
    def compile_timepoints(
        ...
        Args:
            ...
            waveform_array: An empty waveform assembly data structure.
        ...
        num_timepoints = waveform_block.samples
        ...
        for loc, waves in locations.items():
            for w in waves:
                ...
                for i, c in enumerate(w.channels):
                    try:
                        waveform_block.get_waveform(channel_group[c])[
                            s_idx:e_idx
                        ] += w_t[i]
                    except KeyError:
                        continue

                if isinstance(w, TriggeredWaveform):
    ...
    def compile_sequence_element(
        ...
        locations = se.resolve_locations(
            end_marker=self.end_marker, markers=markers, **location_kwargs
        )

        # Compile phases
        phase_tracker = self.compile_phases(locations)
        t_end = markers[self.end_marker].offset

        outputs = {}

        for name, channel_group in self.channels.items():
            num_timepoints = int(t_end * channel_group.sample_rate)

            wblk = WaveformBlock(
                samples=num_timepoints, sample_rate=channel_group.sample_rate
            )

            # Skip compilation for channel group if no waveforms on channel
            if se.channels & channel_group.channel_names():
                self.compile_timepoints(
                    locations=locations,
                    waveform_block=wblk,
    ...
    def compile(
        ...
        Args:
            ...
            **triggered_elements: Any additional triggered elements.

        Returns:
            A `CompiledSequence` instance.
        """
        exe = QWiPExecutable(sequence=seq)
        ...
        for se in seq.flat:
            outputs = self.compile_sequence_element(
                se, exe, location_kwargs, pulse_kwargs, waveform_cache
            )

            for name, wblk in outputs.items():
                if not self.channels[name].triggered:
                    exe.devices[name].append(wblk)
