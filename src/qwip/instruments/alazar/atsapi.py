...
This module provides a thin wrapper on top of the AlazarTech C API. All the exported 
methods directly map to underlying C functions. Please see the ATS-SDK Guide for detailed
specification of these functions. In addition, this module provides a few classes for
convenience.
"""
...
PyCType = type(c_uint8)

## ======== Clocks ======== ##
# Available sources for a given board form a subset of this class' members.
# Please see your board's specification as well as the ATS-SDK manual for more
# information.
INTERNAL_CLOCK = 0x1
EXTERNAL_CLOCK = 0x2
...
## ======== Sample Rates ======== ##
# Sample rates that the internal clock of a board can generate. Available sample
# rates for a given board form a subset of this class' members. Please see your board's
# specification as well as the ATS-SDK manual for more information.
...
# Direction of the edge from the external clock signal that the board syncrhonises with.
CLOCK_EDGE_RISING = 0
...
# Note: Available input ranges for a given board and a given configuration form a
# subset of this class' members. Please see your board's specification as well as
# the ATS-SDK manual for more information.
...
## ======== Trigger Edge ======== ##
# Edge of the external trigger signal that the board synchronizes with."""
TRIGGER_SLOPE_POSITIVE = 1
TRIGGER_SLOPE_NEGATIVE = 2

## ======== Impedance Identifiers ======== ##
# Available parameters for a given board form a subset of this class' members.
# Please see your board's specification as well as the ATS-SDK manual for more
# information.
IMPEDANCE_1M_OHM = 1
IMPEDANCE_50_OHM = 2
...
class DMABuffer:
    ...
    AlazarTech digitizers use direct memory access (DMA) to transfer data from digitizers
    to the computer's main memory. This class abstracts a memory buffer on the host, and
    ensures that all the requirements for DMA transfers are met.

    DMABuffers export a 'buffer' member, which is a NumPy array view
    of the underlying memory buffer

    Args:
        c_sample_type: The datatype of the buffer to create.
        size_bytes: The size of the buffer to allocate, in bytes.
    """

    def __init__(self, c_sample_type: PyCType, size_bytes: int):
...
ats.AlazarExtractNPTFooters.argtypes = [
    c_void_p,
    c_uint32,
    c_uint32,
    POINTER(NPTFooter),
...
class Board:
    ...
    Args:
        systemId: The board system identifier of the targetboard. Defaults to 1, which is
            suitable when there is only one board in the system.
        boardId: The target's board identifier in it's system. Defaults to 1, which is
            suitable when there is only one board in the system.
    """

    def __init__(self, systemId: int = 1, boardId: int = 1):
    ...
    def getChannelInfo(self):
        ...
        Returns the on-board memory in samples per channel and sample size in bits per
        sample.
        """
    ...
    ats.AlazarCreateStreamFileA.restype = U32
    ats.AlazarCreateStreamFileA.argtypes = [U32, c_char_p]
    ats.AlazarCreateStreamFileA.errcheck = returnCodeCheck

    def createStreamFile(self, filePath):
        """Create a file to stream data to harddrive"""
        ats.AlazarCreateStreamFileA(
            self.handle, filePath
        )  # Python uses ASCII string coding by default. Hence the 'A'
    ...
    def waitNextAsyncBufferComplete(self, buffer, bytesToCopy, timeout_ms):
        """Returns when the next buffer is filled.

        Use this with the ADMA_ALLOC_BUFFERS flag, which automatically allocates buffers.
        """
        ats.AlazarWaitNextAsyncBufferComplete(
            self.handle, buffer, bytesToCopy, timeout_ms
        )

    ats.AlazarGetStatus.restype = U32
    ats.AlazarGetStatus.argtypes = [U32]
    ats.AlazarGetStatus.errcheck = returnCodeCheck
