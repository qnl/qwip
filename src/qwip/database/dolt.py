...
def dolt_checkout(connection, target: str, new_branch: bool = False) -> None:
...
class DoltTable(Table):
    ...
    def commit(
        ...
        dolt_commit(
            connection, message, author=author, date=date, allow_empty=allow_empty
        )
    ...
    def create_history_table(self, metadata=None):
        ...
        if name in metadata.tables:
            raise ValueError(
                f"Table {name} already exists in metadata object: {metadata.tables}."
            )

        history_table = self.to_metadata(metadata, name=name)
    ...
    def create_diff_table(self, metadata=None):
        ...
        to_columns = [
            Column(f"to_{col.name}", col.type, primary_key=col.primary_key, system=True)
    ...
    def create_blame_table(self, metadata=None):
        ...
        pk_columns = [
            Column(col.name, col.type, primary_key=True, system=True)
            for col in self.columns
            if col.primary_key
        ]
